...
DecodeDigitallySigned(Reader& reader, DigitallySigned& output)
{
  ...
  Result rv = ReadHashAlgorithm(reader, output.hashAlgorithm);
  if (rv != Success) {
    return rv;
  }
  rv = ReadSignatureAlgorithm(reader, output.signatureAlgorithm);
  if (rv != Success) {
    return rv;
  }
  ...
  rv = InputToBuffer(signatureData, output.signatureData);
  if (rv != Success) {
    return rv;
  }
                                 ...
                                 SignedCertificateTimestamp& output)
{
  ...
  Result rv = ReadVersion(reader, output.version);
  if (rv != Success) {
    return rv;
  }
  ...
  rv = DecodeDigitallySigned(reader, output.signature);
  if (rv != Success) {
    return rv;
  }

  rv = InputToBuffer(logId, output.logId);
  if (rv != Success) {
    return rv;
  }
  rv = InputToBuffer(extensions, output.extensions);
  if (rv != Success) {
    return rv;
  }
